...
window.Exporter = (function () {
  ...
  async function monthlyExcel(report, ordersAll, monthLabel) {
    ...
    const head1 = ['设计师', '角色', '接单数', '定稿数', '定稿率', '完成率', '绩效系数',
      '小单有效数', '基础绩效(元)', '小单提成(元)', '小单扣减(元)', '总绩效(元)'];
    const rows1 = report.rows.map(r => [
      r.designerName, r.role, r.total, r.finalizedCount, pct(r.rate), pct(r.completion),
      r.coef, r.smallCount, money(r.basePerf), money(r.smallBonus), money(r.smallDeduction), money(r.totalPerf)
    ]);
    rows1.push(['', '', '', '', '', '', '', '', '', '', '', '']);
    rows1.push(['团队汇总', '', '', '', '', '', '', '',
      '团队营收(元): ' + money(report.team.revenue) + ' · 人均小单: ' + report.team.avgSmall]);
    rows1.push(['团队奖(元)', '', '', '', '', '', '', '', '', '', '', money(report.team.award)]);
    rows1.push(['绩效合计(元)', '', '', '', '', '', '', '', '', '', '', money(report.team.totalPerfSum)]);
    const aoa1 = [['设计部业绩月报表 — ' + monthLabel + '（考核窗口：' + winStart + ' ~ ' + winEnd + '）'], head1].concat(rows1);

    // —— Sheet2 订单明细（窗口内）——
    const head2 = ['订单号', '项目', '客户', '任务类型', '金额(元)', '分类',
    ...
    const rows2 = winOrders.map(o => [
      ...
      designerName(o.assigned_designer_id), (window.Cfg.STATUS[o.status] || {}).label || o.status, o.revision_count || 0,
  ...
  async function ordersCSV(orders, designers, settings, filename) {
    const head = ['订单号', '项目', '客户', '任务类型', '金额(元)', '分类', '设计师',
      '状态', '修改次数', '是否定稿', '接单时间', '派单时间', '截稿时间', '定稿时间', '备注'];
    const dName = id => (designers.find(d => d.id === id)?.name) || '';
    const rows = orders.map(o => [
      o.order_no, o.title, o.customer_name || '', o.task_type, money(o.amount),
      window.Cfg.orderCategory(Number(o.amount) || 0, settings),
      dName(o.assigned_designer_id), (window.Cfg.STATUS[o.status] || {}).label || o.status, o.revision_count || 0,
      (o.status === '已定稿' && (o.revision_count || 0) <= 1) ? '是' : '否',
