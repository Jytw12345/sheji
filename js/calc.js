...
window.Calc = (function () {
  ...
  // 给定参考日期 refDate，返回「包含该日期」的考核窗口 [start, end]
  // settings.win_start_day / win_end_day 可配置（默认 26 / 25，即本月26日~下月25日为一个窗口）
  function addMonths(date, n) {
    const d = new Date(date);
    d.setMonth(d.getMonth() + n);
    return d;
  }
  function windowOf(refDate, settings) {
    const ref = refDate ? new Date(refDate) : new Date();
    const startDay = (settings && Number(settings.win_start_day)) ? Number(settings.win_start_day)
                    : (window.Cfg.win_start_day || 26);
    const endDay = (settings && Number(settings.win_end_day)) ? Number(settings.win_end_day)
                  : (window.Cfg.win_end_day || 25);
    // 找到包含 ref 的窗口：
    // 若 ref.day >= startDay，窗口从 ref 所在月的 startDay 开始，到下月 endDay 结束；
    // 若 ref.day <  startDay，窗口从 ref 所在月上月的 startDay 开始，到 ref 所在月的 endDay 结束。
  ...
  async function monthlyReport(reportMonthDate, scopeDesignerId) {
    ...
    const win = windowOf(reportMonthDate, settings);
  ...
  // 仪表盘用的“当前月/上月”概览（含订单状态/类型分布）
  // reportMonthDate: 报告月，默认当前月；传入上月即得到「上期」数据
  async function dashboardSummary(reportMonthDate) {
    reportMonthDate = reportMonthDate || new Date();
    ...
    // 状态分布（当前考核窗口内，随本期/上期切换）
    const statusDistWin = {};
    winOrders.forEach(o => { statusDistWin[o.status] = (statusDistWin[o.status] || 0) + 1; });

    // 类型分布（当前考核窗口内）
    const typeDistWin = { '小单': 0, '普通': 0, '大单': 0 };
    winOrders.forEach(o => {
      const c = window.Cfg.orderCategory(Number(o.amount) || 0, settings);
      typeDistWin[c] = (typeDistWin[c] || 0) + 1;
    });

    // 设计师业绩（窗口内营收）与提成
    const perf = await monthlyReport(reportMonthDate);
    ...
    return {
      ...
      statusDist, typeDist, statusDistWin, typeDistWin, designerPerf,
   ...
   *  - 设计返工率 = 已定稿中经历过程改(revision_count>0)的订单 ÷ 已定稿数（近似"需返工率"；rework_category 已不再采集）
  ...
  function rangeOf(opts, settings) {
    ...
    // 考核窗口：默认本期（当前月），可切换为上期（上月）
    const reportMonth = (opts && opts.period === 'previous')
      ? addMonths(new Date(), -1)
      : new Date();
    return windowOf(reportMonth, settings);
  }
  ...
  // 逾期/延期风险预测：基于设计师历史平均定稿周期，估算在建单「预计完成时间」，对比 deadline 分四级
  //   red    = 已逾期 或 预计完成晚于 deadline（必逾期）
  //   yellow = 预计完成落在 deadline 前「风险缓冲」天数内（临界）
  //   green  = 预计完成远早于 deadline（正常）
  //   none   = 已完成 / 无截稿 / 未排期（不评）
  function assessRisk(ordersAll, designers, settings) {
    // 1) 每位设计师的历史平均定稿周期（仅已定稿单，复用 cycleDays）
    const cycleByDesigner = {};
    designers.forEach(d => {
      const finalized = ordersAll.filter(o => o.status === '已定稿' && window.Cfg.participants(o).includes(d.id));
      let sum = 0, n = 0;
      finalized.forEach(o => { const c = cycleDays(o); if (c != null) { sum += c; n++; } });
      cycleByDesigner[d.id] = n ? sum / n : null;
    });
    // 2) 团队平均（设计师无历史数据时兜底）
    const allF = ordersAll.filter(o => o.status === '已定稿');
    let tSum = 0, tN = 0;
    allF.forEach(o => { const c = cycleDays(o); if (c != null) { tSum += c; tN++; } });
    const teamAvg = tN ? tSum / tN : 7; // 无任何历史定稿单时默认 7 天
    // 3) 风险缓冲天数：预计完成落在 deadline 前 buf 天内视为「临界」（可在设置页调整）
    const buf = Number(settings && settings.risk_buffer_days) || 1.5;
    // 4) 临近窗口：截稿剩余 > 该天数视为「还早」，不进入超期评估，避免误伤正常单
    //    取 7 天与团队平均周期的大者，长周期团队窗口更大（否则远处长单也会被漏评）
    const nearWin = Math.max(7, Math.ceil(teamAvg));
    // 5) 状态进度比例：按当前推进程度估算「剩余所需天数」，而非从零算全程周期
    const progressByStatus = {
      '接单': 0, '派单': 0.1, '提案': 0.2, '提案不通过': 0.2,
      '设计中': 0.4, '初稿': 0.55, '客户反馈': 0.7, '修改中': 0.85
    };
    const now = Date.now();
    const DAY = 86400000;
    const out = {};
    ordersAll.forEach(o => {
      ...
      if (done) { out[o.id] = { level: 'none', reason: '已完成' }; return; }
      if (!o.deadline) { out[o.id] = { level: 'none', reason: '无截稿' }; return; }
      // 未派单（还没开始做）不评超期，避免「新建即标红」误伤；临期由弹窗彩色提示覆盖
      if (!o.dispatch_at) { out[o.id] = { level: 'none', reason: '未派单' }; return; }
      const dl = new Date(o.deadline).getTime();
      const daysLeft = (dl - now) / DAY;
      // 截稿还远：直接判正常，不进入超期评估
      if (daysLeft > nearWin) { out[o.id] = { level: 'green', reason: '周期充裕' }; return; }
      const owner = o.assigned_designer_id;
      const avg = (owner && cycleByDesigner[owner] != null) ? cycleByDesigner[owner] : teamAvg;
      const progress = progressByStatus[o.status] != null ? progressByStatus[o.status] : 0;
      const remaining = avg * (1 - progress); // 剩余所需天数（按当前进度折算）
      const estFinish = now + remaining * DAY;
      let level, reason;
      if (now > dl) { level = 'red'; reason = '已逾期'; }
      else if (estFinish > dl) { level = 'red'; reason = '预计超期'; }
      else if (estFinish > dl - buf * DAY) { level = 'yellow'; reason = '临界'; }
      else { level = 'green'; reason = '正常'; }
      out[o.id] = {
        level, reason,
        estFinish: new Date(estFinish).toISOString(),
        daysLeft: Math.round(daysLeft * 10) / 10,
        avgCycleUsed: Math.round(avg * 10) / 10,
  ...
  async function analytics(opts, scopeDesignerId) {
    ...
    const range = rangeOf(opts, settings);
    const s = settings;
    ...
    // ---------- 团队级汇总 ----------
    const intakeOrders = src.filter(o => inRangeAt(o, 'intake_at', range) && o.status !== '已取消');
    ...
    // 已取消订单（客户中途终止）：不计入定稿/完成/提案等率分母，但单独统计取消率
    const cancelledOrders = dispatchOrders.filter(o => o.status === '已取消');
    const effectiveDispatch = dispatchOrders.filter(o => o.status !== '已取消');
    const finalizedAll = dispatchOrders.filter(isFinalized);

    // 提案（团队）
    const proposalResolvedAll = effectiveDispatch.filter(o =>
      isFinalized(o) || (o.revision_count || 0) >= 1 || o.status === '已换人');
    const proposalPassAll = effectiveDispatch.filter(o =>
      o.status === '已定稿' && (o.revision_count || 0) === 0).length;
    // 设计返工率（新口径）：已定稿订单中「经历过程改(revision_count>0)」的占比。
    // 原口径依赖 rework_category（设计原因/客户原因），但该字段已不再由流程写入，恒为空→恒为 0。
    // 现改用 revision_count（客户反馈→修改中 时自动 +1）近似「需返工的单 ÷ 已定稿」，无需新增录入。
    const reworkedAll = finalizedAll.filter(o => (o.revision_count || 0) > 0).length;
    const finalizedAllAny = effectiveDispatch.filter(o => o.status === '已定稿');

    // 一次提案通过率（团队）：首次提案就一次性通过 ÷ 已决提案
    //   已决提案 = 有过 proposal_pass_at 或 proposal_failed_at（提案环节已有明确结论）
    //   首单通过 = 提案次数<=1 且从未触发过「提案不通过」
    const proposalDecidedAll = effectiveDispatch.filter(o => o.proposal_pass_at || o.proposal_failed_at);
    ...
    const rows = designersActive.map(d => {
      ...
      const effectiveMine = mineDispatch.filter(o => o.status !== '已取消');
      const dispatchCount = effectiveMine.length;
      ...
      const resolved = effectiveMine.filter(o =>
        isFinalized(o) || (o.revision_count || 0) >= 1 || o.status === '已换人');
      const pass = effectiveMine.filter(o => o.status === '已定稿' && (o.revision_count || 0) === 0).length;
      const proposalPassRate = resolved.length ? pass / resolved.length : 0;

      const reworkedCount = finalizedMine.filter(o => (o.revision_count || 0) > 0).length;
      const reworkRate = finalizedCount ? reworkedCount / finalizedCount : 0;

      const myFinalizedAny = effectiveMine.filter(o => o.status === '已定稿');
      ...
      const proposalDecided = effectiveMine.filter(o => o.proposal_pass_at || o.proposal_failed_at);
    ...
    const perOrder = dispatchOrders.map(o => {
      ...
      return {
        ...
        intake_at: o.intake_at, dispatch_at: o.dispatch_at, deadline: o.deadline,
        finalized_at: o.finalized_at, switched_at: o.switched_at,
        revision_log: o.revision_log || [], complaint_log: [],
        cycleDays: c
      };
    });

    // 经营分析/导出需要展示投诉原因，但主加载列裁剪了 complaint_log，
    // 这里仅对 complaint_count>0 的订单批量补拉一次。
    const complaintIds = perOrder.filter(o => o.complaint_count > 0).map(o => o.id);
    if (complaintIds.length && window.DB && window.DB.loadOrderComplaintLogs) {
      const complaintMap = await window.DB.loadOrderComplaintLogs(complaintIds);
      perOrder.forEach(o => { o.complaint_log = complaintMap[o.id] || []; });
    }
    ...
    return {
      ...
      totals: {
        ...
        draftToFinalizeRate,
        reworkRate: finalizedAll.length ? reworkedAll / finalizedAll.length : 0,
        reworkedCount: reworkedAll,
        currentInProgress,
        cancelRate: (() => { const c = cancelledOrders.length + effectiveDispatch.filter(o => o.status === '已定稿' || o.status === '已换人').length; return c ? cancelledOrders.length / c : 0; })(),
        peakConcurrency,
        // 工资核算（与绩效月报同源）
        teamAward,
        totalPerfSum: Math.round(totalPerfSum * 100) / 100,
  ...
  return { windowOf, inWindow, isFinalized, monthlyReport, dashboardSummary, computeDesigner, analytics, addMonths, assessRisk };
})();
