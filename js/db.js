...
window.DB = (function () {
  ...
  // 【v484】Single Flight：同 key 的并发请求合并到一个 in-flight Promise，避免重复打 Supabase。
  //   典型收益：realtime 触发 reconcile 与 reload 并发同表查询时，只发一次。
  const _flight = new Map();
  function flight(key, fn) {
    if (_flight.has(key)) return _flight.get(key);
    const p = Promise.resolve().then(fn).finally(() => _flight.delete(key));
    _flight.set(key, p);
    return p;
  }
  // 跨会话持久化上次同步时间：启动即恢复真实「上次同步」时刻，
  // 界面一出来就能显示「已同步 X分钟/小时前」，无需等本次云端 reload 把 lastSync 刷成「刚刚」。
  let lastSync = new Date(lsGet('ds_last_sync', Date.now()));
  ...
  function emit() {
    lastSync = new Date();
    lsSet('ds_last_sync', lastSync.getTime());
  ...
  function markSynced() { lastSync = new Date(); lsSet('ds_last_sync', lastSync.getTime()); }
  ...
  async function init() {
    ...
    // 显式 auth 配置（v484）：
    //  - storageKey 自定义避免与项目 ref 绑定（换部署不丢）
    //  - persistSession 持久化到 localStorage（默认就是 true，显式声明稳）
    //  - autoRefreshToken 默认 true，显式声明
    //  - detectSessionInUrl=false：纯云端 SPA，不走 URL hash 回传，避免奇怪的 #access_token=... 残留
    //  - flowType='pkce'：明确 OAuth 流程类型（邮箱密码无差异，但显式更稳）
    ...
    // （v487）「令牌即将过期预刷新」已移除：与 autoRefreshToken:true 功能重复，且对启动/登录延迟无帮助；
    //   令牌过期前由 SDK 自动续期兜底，不会出现「操作中突然 401 被踢回登录」的回归。
  ...
  // 【v479 优化】orders 主加载列裁剪：去掉两个最重的 jsonb（flow_history / complaint_log），
  //   它们仅在详情弹窗用到，主列表/工作台/分析/看板从来不读，丢掉 80%+ 的单行 jsonb 体积。
  //   详情弹窗打开时由 DB.loadOrderDetail(id) 按需补拉。
  const ORDER_LEAN_COLS = [
    'id','order_no','title','customer_id','customer_name','task_type','amount','status',
    'assigned_designer_id','collab_designer_ids','collab_share_ratio',
    'revision_count','rework_category','is_finalized','complaint_count',
    'intake_at','dispatch_at','deadline','design_started_at','draft_at',
  ...
  //   .catch 静默吞掉 → 用户看"已同步 X分钟前"+缓存旧数据，从 v442 loadLive 落地起就一直有此 bug，
  //   直到 v482 把错误抛到 toast 才被肉眼看见）。
  // 各表独立加载兜底：单表查询失败（如 orders 偶发超时 / RLS 抖动）只返回 null，不连累其他表。
  ...
  async function loadAll() {
    ...
    // 【v484】单表用 flight 合并：与 realtime 触发的 reconcile 撞同表时只发一次查询。
    const [designers, groups, customers, orders, st] = await Promise.all([
      safe('designers', () => flight('loadAll:designers', () => sb.from('designers').select('*'))),
      safe('groups',    () => flight('loadAll:groups',    () => sb.from('groups').select('*').is('deleted_at', null))),
      // 软删除：拉取时过滤掉已移入回收站（deleted_at 非空）的记录，已删数据不出现在主列表
      safe('customers', () => flight('loadAll:customers', () => sb.from('customers').select('*').is('deleted_at', null))),
      safe('orders',    () => flight('loadAll:orders',    () => sb.from('orders').select(ORDER_LEAN_COLS).is('deleted_at', null))),
      safe('settings',  () => flight('loadAll:settings',  () => sb.from('settings').select('*').eq('id', 1).maybeSingle()))
    ]);
  ...
  // 轻量刷新：只拉取会频繁变化的业务数据（订单/客户），静态表（设计师/分组/设置）沿用缓存。
  // 用于「已有完整缓存」后的后台补拉、回前台、自动派单前的快速对账等高频场景，
  // 避免每次都全量 select('*') 5 张表（含几乎不变的配置表），把「旧→新」的等待聚焦到真正会变的数据上。
  async function loadLive() {
    const [customers, orders] = await Promise.all([
      safe('customers', () => flight('loadLive:customers', () => sb.from('customers').select('*').is('deleted_at', null))),
      safe('orders',    () => flight('loadLive:orders',    () => sb.from('orders').select(ORDER_LEAN_COLS).is('deleted_at', null)))
    ]);
    cache.customers = mergeServer('customers', customers && customers.data);
    cache.orders = mergeServer('orders', orders && orders.data);
    markSynced();
  ...
  function setupRealtime() {
    ...
    // 【v479 优化】节流 + per-table reconcile：
    //   原逻辑：任何一张表变动 → 800ms 后 loadAll() 拉全 5 张表（含几乎不变的设计师/分组/设置）。
    //   现逻辑：用 Set 记录「哪几张表」变脏，800ms 后只对变脏的表调 reconcile(单表)。
    //   单端多表同时改 → 仍合并到一次 tick；多端只动一张表 → 只重拉一张，节省 60%+ 带宽。
    //   settings 单行不走 cache，改用 reloadSettings()。
    // 【v481 优化】realtime payload 直写 + 验证重拉：
    //   B) postgres_changes 的 payload 直接写 cache（INSERT/UPDATE/DELETE），绕过 read replica 复制延迟（100-300ms 缩短为即时）
    //   A) 800ms 第一次 reconcile 之后，2.5s 再 reconcile 一次（兜底复制延迟）
    const dirty = new Set();
    let timer = null;
    const verifyTimers = new Map();   // table -> timeout id，2.5s 验证重拉
    TABLES.forEach(t => {
      ch.on('postgres_changes', { event: '*', schema: 'public', table: t },
        (payload) => {
          // === 【v481 B】realtime payload 直写 cache，绕过 read replica 复制 ===
          // 处理 INSERT / UPDATE（合并）/ DELETE / 软删除（UPDATE 但 deleted_at 写入）
          if (payload && (t === 'orders' || t === 'customers' || t === 'groups' || t === 'designers')) {
            const arr = cache[t];
            if (Array.isArray(arr)) {
              if (payload.eventType === 'INSERT' && payload.new && payload.new.id) {
                if (!arr.find(r => r.id === payload.new.id)) {
                  arr.unshift(payload.new);
                  emit();   // 立即让 UI 看到新行（不再等 read replica）
                }
              } else if (payload.eventType === 'UPDATE' && payload.new && payload.new.id) {
                const i = arr.findIndex(r => r.id === payload.new.id);
                if (i >= 0) {
                  // 软删除（deleted_at 写入）等价于从主列表移除；designers 不用 deleted_at
                  if (payload.new.deleted_at && t !== 'designers') {
                    arr.splice(i, 1);
                  } else {
                    arr[i] = Object.assign({}, arr[i], payload.new);
                  }
                  emit();
                } else if (!payload.new.deleted_at || t === 'designers') {
                  // 之前因复制延迟不在 cache 的行被还原/补全，添加
                  arr.unshift(payload.new);
                  emit();
                }
          ...
          } else if (t === 'settings' && payload && payload.new) {
            // settings 是单行表，不在 cache 里，直接更新内存 + 落盘
            settings = Object.assign({}, settings, payload.new);
          ...
          timer = setTimeout(async () => {
            ...
            dirty.clear();
            // 清除该批次的验证重拉计时器（避免和正常 reconcile 重复拉）
            for (const tbl of tables) {
              const vt = verifyTimers.get(tbl);
              if (vt) { clearTimeout(vt); verifyTimers.delete(tbl); }
            }
            try {
              for (const tbl of tables) {
                if (tbl === 'settings') {
            ...
            } catch (e) { console.warn('[Realtime] per-table reconcile 失败：', e); }
            emit();
            // 【v481 A】2.5s 后验证重拉：read replica 复制延迟偶发超过 800ms，
            //   这次兜底确保新数据 1-3s 窗口内必出
            for (const tbl of tables) {
              if (verifyTimers.has(tbl)) clearTimeout(verifyTimers.get(tbl));
              verifyTimers.set(tbl, setTimeout(async () => {
                verifyTimers.delete(tbl);
                try {
                  if (tbl === 'settings') await reloadSettings();
                  else await reconcile(tbl);
                } catch (e) { console.warn('[Realtime] 验证重拉失败（不影响主显示）', e); }
              }, 2500));
            }
          }, 800);
        });
  ...
  async function reloadSettings() {
    return flight('settings:reload', async () => {
      try {
  ...
  async function loadDesignersRobust() {
    ...
    for (let i = 0; i < 6; i++) {
      try {
        const { data } = await flight('auth:getSession', () => sb.auth.getSession());
        if (data && data.session && data.session.user) break;
      } catch (e) {}
      if (i < 5) await new Promise(r => setTimeout(r, 250));
    }
    ...
    for (let i = 0; i < 4; i++) {
      try {
        const { data, error } = await flight('loadDesignersRobust:q', () => sb.from('designers').select('*'));
        if (!error && data) {
          cache.designers = mergeServer('designers', data);
  ...
  async function reconcile(table) {
    return flight('reconcile:' + table, async () => {
      try {
        let q = sb.from(table).select(table === 'orders' ? ORDER_LEAN_COLS : '*');
  ...
  // 【v479 优化】详情按需补拉：loadAll/loadLive/reconcile 都不再取 flow_history/complaint_log
  // （仅详情弹窗用），打开详情时由本函数 select('*') 拉全行并合并到 cache，避免脏读
  async function loadOrderDetail(id) {
    if (!id) return null;
    const { data, error } = await sb.from('orders').select('*').eq('id', id).maybeSingle();
    if (error) { console.warn('[loadOrderDetail] 失败：', error.message); return null; }
    if (data) {
      const i = cache.orders.findIndex(r => r.id === id);
      if (i >= 0) cache.orders[i] = Object.assign({}, cache.orders[i], data);
    }
    return data;
  }
  // 批量补拉订单投诉记录：经营分析明细表/导出需要显示投诉原因，但主加载列裁剪了 complaint_log。
  // 仅对 complaint_count>0 的订单按 id 批量查询，避免全量 select('*') 拖慢分析。
  async function loadOrderComplaintLogs(ids) {
    if (!sb || !ids || !ids.length) return {};
    const map = {};
    try {
      const res = await flight('loadOrderComplaintLogs', async () => {
        const all = [];
        for (let i = 0; i < ids.length; i += 50) {
          const slice = ids.slice(i, i + 50);
          const { data, error } = await sb.from('orders').select('id,complaint_log').in('id', slice);
          if (error) throw error;
          if (data) all.push(...data);
        }
        return all;
      });
      (res || []).forEach(o => { map[o.id] = Array.isArray(o.complaint_log) ? o.complaint_log : []; });
    } catch (e) { console.warn('[loadOrderComplaintLogs] 失败：', e.message); }
    return map;
  }
  ...
  // 【v540】跨应用实时提醒：订阅抢单平台(xinxifabu)新需求发布。
  // 前提：dr_requirements 与工作台同库（Supabase 东京）、已加入 supabase_realtime 发布、
  //       RLS(dr_req_select) 允许登录用户 SELECT。仅监听 INSERT 事件，不写工作台缓存。
  function subscribeDrNewReq(cb) {
    if (!sb) { console.warn('[dr] sb 未就绪，跳过新需求订阅'); return null; }
    try {
      const ch = sb.channel('dr-new-req')
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'dr_requirements' },
          (payload) => {
            try { if (cb && payload && payload.new) cb(payload.new); }
            catch (e) { console.warn('[dr] 回调异常', e); }
          })
        .subscribe();
      return ch;
    } catch (e) { console.warn('[dr] 新需求订阅失败', e); return null; }
  }

  // 【v540】查询抢单平台最近可抢需求（供工作台提醒面板展示）。RLS 允许登录用户 SELECT 未删需求
  async function listDrRecentRequirements(limit) {
    if (!sb) return [];
    try {
