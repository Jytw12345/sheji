...
(function () {
  ...
  function riskMap() {
    if (!state._riskMap) {
      state._riskMap = window.Calc.assessRisk(state._orders || [], state._designers || [], state._settings || {});
    }
  ...
  function syncSwipeStripVisibility() {
    ...
    if (!isSwipeMode()) { secs.forEach(s => { s.style.display = ''; }); swipeLog('syncSwipe: desktop mode, reset displays'); return; }
  ...
  async function bootAuth() {
    ...
    if (session && session.user) {
      // 用已认证身份全量重拉数据（v245 后 init 仅拉取 designers，其余表为空）
      try { await DB.reload(); await loadData(); } catch (e) {}
  ...
  async function renderDashboard() {
    await ensureChartLib(); // 确保 Chart.js 就绪（首屏已 defer 预载，这里仅兜底）
  ...
  function deadlineFieldsHtml(o) {
    ...
    const hh = time ? time.slice(0, 2) : '12';
  ...
  function renderOrderModal() {
    ...
    if (can('flow_revert') && canRevert(o.status)) {
      const isTerminal = (o.status === '已定稿' || o.status === '已换人');
      if (isTerminal) {
        flow += '<span class="revert-link" data-flow="revert" title="撤销最近一步流程推进">↩ 回退一步</span>';
      } else {
        flow += '<button class="btn warn ghost" data-flow="revert" title="撤销最近一步流程推进（按权限开放）">↩ 回退一步</button>';
      }
    }
  ...
  async function advanceFlow(action) {
    ...
    if (action === 'revert') {
      ...
      if (!(await uiConfirm('确定要回退这一步吗？\n将撤销最近一步流程推进，恢复到上一步状态。此操作需重新保存。'))) return;
  ...
  function nextActionText(status) {
    const map = {
      ...
      '设计中': '提交初稿', '初稿': '客户反馈', '客户反馈': '定稿或修改', '修改中': '修改完成后定稿',
      '已定稿': '已完成', '已换人': '已换人'
    };
  ...
  async function exportAnaCSV() {
    ...
    const head = ['设计师', '角色', '派单量', '定稿率', '一次提案通过率', '初稿定稿率', '平均定稿时间', '设计返工率', '定稿数', '完成率', '当前在制', '峰值并发', '小单有效', '营收', '系数'];
    const rows = rep.rows.map(r => [
      r.designerName, r.role, r.dispatchCount, pct(r.finalizeRate), pct(r.firstProposalPassRate),
      pct(r.draftToFinalizeRate), r.avgCycle ? fmtCycle(r.avgCycle) : '—', pct(r.reworkRate),
