...
(function () {
  ...
  let state = {
    ...
    _isSyncing: false,           // 【v479b】后台拉数据期间为 true：updateSync 据此显示「正在同步…」而非「已同步 刚刚」，
                                 //   避免指示器比数据先到，UI 出现「刚刚 + 旧数据」的错位（实测 1-3s 同步窗口里偶发）。
    _syncError: null,            // 【v482】后台同步失败时填入错误对象，updateSync 据此显示「同步失败」红字状态；用户能感知、能点击重试。null = 无错误
    orderPage: 1,
    customerPage: 1,
    ...
    _newReqChannel: null,       // realtime 频道引用（防重复订阅/调试）
    _newReqLastSeen: null       // 上次查看提醒面板时间戳（ISO）
  };
  ...
  /* 返工周期（天）：首轮「需要修改」→ 定稿。revision_log[0] 为首轮修改时间；无修改或尚未定稿返回 null */
  function reworkDays(o) {
    if (!o || (o.revision_count || 0) <= 0) return null;
    const log = Array.isArray(o.revision_log) ? o.revision_log : [];
    const start = log.length ? log[0] : o.revision_at;
    const end = o.finalized_at;
    if (!start || !end) return null;
    const d = (new Date(end).getTime() - new Date(start).getTime()) / 86400000;
    return d > 0 ? d : null;
  }
  /* 投诉原因汇总：取订单 complaint_log 的 reason 去重后 / 连接；无投诉返回 null */
  function complaintReasons(o) {
    if (!o || (o.complaint_count || 0) <= 0) return null;
    const log = Array.isArray(o.complaint_log) ? o.complaint_log : [];
    if (!log.length) return null;
    const reasons = [...new Set(log.map(c => String(c.reason || '—').trim()).filter(Boolean))];
  ...
  async function afterAuthLogin() {
    ...
    // reload 必须同步等待：全量数据 + loadAll:231 已把云端真实 settings 拉进内存（JWT 生效即真实）。
    // loadSettingsRobust 改为后台 fire-and-forget（下方 .then）：它是 JWT 尚未对 PostgREST 生效时的
    //   重试保险（最坏 ~1.6s），原本在 hideSplash 之前就 await 阻塞整条登录链路，是首登「登录中」6s 的真大头。
    //   首屏只用 reload 拉到的 settings（未生效则用 init 阶段从 localStorage 合并的上次真实权限），
    //   后台 loadSettingsRobust 拿到云端真实值后无感重算 applyPermissions，避免回归「登录后默认权限」bug。
    // 下游「未匹配到设计师档案则 loadDesignersRobust 重试」不变（仅 me 为空时触发，正常路径不阻塞）。
    try {
      await DB.reload();
      DB.loadSettingsRobust().then(async () => {
        try {
          state._settings = await DB.getSettings();   // 取最新真实权限
          applyPermissions();
          renderUserBox();
          if (!state.tab || !can(tabPermKey(state.tab))) state.tab = 'dashboard';
          updateSync();
        } catch (e) { console.warn('[auth] 后台权限对账失败', e); }
      }).catch(e => console.warn('[auth] 后台 loadSettingsRobust 失败', e));
      await loadData();
    } catch (e) {
      // 【v482】把错误暴露给用户，避免和 afterLogin Path A 一样被静默吞掉
      state._syncError = e;
      console.warn('登录后重载数据失败', e);
      const msg = (e && (e.message || String(e))) || '未知错误';
      const isAuth = /401|jwt|token|auth/i.test(msg);
      toast(isAuth ? '同步失败：登录已过期，请重新登录' : '同步失败：' + msg + '（点击指示器重试）', 'warn', 6000);
    }
  ...
  async function doLogin(email, pw) {
    ...
    try {
      ...
      try {
        ...
        // 登录成功：清空失败计数（v484 改为非阻塞 RPC，与 afterAuthLogin 并行，省一次 RTT ~100-300ms）。
        // 失败也无妨，下次再清。
        DB.auth.clearLoginFailures(email).catch(() => {});
  ...
  async function afterLogin() {
    ...
    // 【v540】订阅抢单平台新需求实时提醒（登录后建立，登录前不打扰）
    ...
    if (hadCachedData) {
      ...
      state._syncError = null;   // 【v482】开始新一轮同步，清空上次的失败状态
      DB.reload({live:true}).then(async () => {
        // 轻量补拉（仅订单/客户，静态表用缓存）完成，刷新状态与视图
        ...
        // 【v479b】后台同步真正完成（state 已更新、视图已就绪）才解锁指示器，
        //   避免「刚刚 + 旧数据」的错位；同时 updateSync 内的 _isSyncing 分支负责恢复「已同步 刚刚」
        state._isSyncing = false;
        state._syncError = null;   // 【v482】成功路径也清空失败状态
        // 【v441】后台自动同步完成，刷新「已同步」时钟（loadAll 内部已 markSynced）
        updateSync();
      }).catch(e => {
        // 即便失败也要解锁指示器，否则「同步中」会卡住
        state._isSyncing = false;
        // 【v482】核心修复：把错误暴露给用户（toast + 指示器红字「同步失败」+ cursor:pointer）
        //   之前只 console.warn，用户看着"已同步 18分钟前"误以为成功，订单数停留在缓存旧值。
        state._syncError = e;
        console.warn('后台全量补拉失败（保留当前视图）', e);
        // 401/会话过期：toast 里直接说明，便于用户判断
        const msg = (e && (e.message || String(e))) || '未知错误';
        const isAuth = /401|jwt|token|auth/i.test(msg);
        toast(isAuth ? '同步失败：登录已过期，请重新登录' : '同步失败：' + msg + '（点击指示器重试）', 'warn', 6000);
        updateSync();
      ...
      // 正常情况下 afterAuthLogin 已用「已认证身份」DB.reload() + loadData() 拉好全量数据（state 已填充）。
      // 这里只处理"空数据"兜底补拉；渲染不再 await，否则 renderDashboard 里的 ensureChartLib()
      // （chart.js ~1MB）会把 hideSplash 堵住 2-3s，导致"登录中"结束后菜单仍 6-7 秒点不动。
      // 改为后台渲染：首屏/逾期 badge/菜单立即可交互，图表库在后台加载完成后自动重绘。
      const ready = ((state._orders && state._orders.length) || (state._customers && state._customers.length));
      if (!ready) {
        try {
          await DB.reload();
          state._designers = await DB.listDesigners(); state._customers = await DB.listCustomers();
      ...
      persistBusinessCache();   // 写入完整缓存（数据已在 state 中，无论本分支是否补拉），下次冷启动走 Path A 秒进
      // 不阻塞 hideSplash：renderTabContent 内部 await ensureChartLib / dashboardSummary，
      // 若在此处 await，会把整段登录链路拖长。
      renderTabContent(state.tab).catch(e => console.error('[afterLogin Path B] 渲染失败', e));
      hideSplash();                        // 数据已就绪，立即揭开 Splash
    }
  ...
  async function bootAuth(preSession) {
    let session = preSession;
    if (!session) {
      try { session = await DB.auth.getSession(); } catch (e) { session = null; }
    }
    if (session && session.user) {
      // 业务数据不在此预拉：登录后由 afterLogin 的 Path B(DB.reload) / Path A(缓存) 补全；本函数仅用 DB.init 已就绪的设计师列表定位当前用户。
      let me = (state._designers || []).find(d => d.auth_id && d.auth_id === session.user.id);
  ...
  // 【v491】最小展示时长：盖住首屏「空仪表盘→数据弹出」的尴尬——
  //   前台保持启动画面，后台继续拉取/刷新（DB.reload / loadSettingsRobust / renderTabContent 均已在后台跑），
  //   让用户在「启动画面」下等完这 ~1.3s，而不是对着半空仪表盘等数据。
  //   仅在数据比该值更快加载完时才延后揭面；慢于此的加载（冷启动 4s+）不会被延长，仅由 25s 兜底超时负责。
  const _SPLASH_MIN_MS = 1200;       // 启动画面最短展示时长（用户要求 ~1~1.5s，单行可调）
  let _splashShownAt = Date.now();   // 取 app.js 解析时近似首屏可见时刻
  let _splashHideTimer = null;
  function hideSplash() {
    const sp = document.getElementById('bootSplash');
    if (!sp || sp.dataset.hidden) return;          // 已隐藏则忽略（幂等）
    const remain = Math.max(0, _SPLASH_MIN_MS - (Date.now() - _splashShownAt));
    if (remain > 0) {
      if (_splashHideTimer) clearTimeout(_splashHideTimer);
      _splashHideTimer = setTimeout(_doHideSplash, remain);
    } else {
      _doHideSplash();
    }
  }
  ...
  async function init() {
    ...
    // 【v535 修复】把 bindTabs + bindGlobal 从原 await DB.init() 之后移到此处之前。
    //   根因：bootApp Path A 下仪表盘、菜单立即可见（缓存数据），但菜单点击 handler 在
    //   init() await DB.init() 之后才绑定——DB.init 走冷 DNS+TLS（东京 2~6s），期间整个
    //   应用不可交互；updateOverdueBadge 也在 init() 后段跑，时间几乎重合，导致用户误以为是
    //   「等顶部逾期出现才能点菜单」。这俩绑定只依赖 DOM（已就绪）+ state（已声明），纯本地，
    //   移到 DB.init 之前立即生效；用 _uiBound 守卫 showBootRetry 重跑 init() 时双绑。
    if (!state._uiBound) { state._uiBound = true; bindTabs(); bindGlobal(); }
    await DB.init();
    // 【v533 修复】DB.init() 内的 loadDesignersOnly 写的是 db.js 内部的 cache.designers，
    // 不会自动回写到 app.js 的 state._designers。v486 启动优化去掉了 init() 里原本的
    // await loadData()（那行会把 cache 同步到 state._designers），但忘了补这条赋值——
    // 导致冷启动登录页 renderLogin() 读到 state._designers=undefined，(all.length?...) 三元把
    // 整段快捷登录吞掉。退出登录时 state._designers 是上次登录残留的快照，所以「退出后才有」。
    // DB.listDesigners() = cache.designers.slice()，纯内存读、零网络；不动 v486 优化效果。
    state._designers = await DB.listDesigners();
    // 【v469 优化】提前触发会话读取 / 令牌静默刷新，与后续本地 loadData、绑定、SW 安装并行，
    // 避免 getSession 串行排在 DB.reload 之前（token 过期冷启动会多一轮 RTT）。
    // 结果在 bootAuth 处复用（传入），不会重复发起刷新。
    const _earlySession = DB.auth.getSession().catch(() => null);
    ...
    // 【v535】bindTabs/bindGlobal 已上移到 init() 开头（详见上方 v535 注释），此处不再调用。
    // 【v486 启动优化】不再 await 冷 loadData() 再 reveal UI：
    // 设计师列表已由 DB.init() 的 loadDesignersOnly 就绪，足以支撑 bootAuth 找 me / 渲染登录快捷按钮。
    // 业务全量数据改为「先揭开首屏/登录界面，再后台拉取」——不再被跨区冷连接（东京 Supabase）挡 6~8 秒，
    // 解决「重启后 6 秒什么都点不动」。登录后 afterLogin 的 Path B 会自行 DB.reload() 补全，
    // 已登录且有缓存走 Path A 用缓存，故 init 内的 loadData 结果对两条路径都冗余（此前还造成重复冷拉取）。
    await bootAuth(await _earlySession);
  ...
  function persistBusinessCache() {
    ...
    try {
      ...
      // 【v481b】容量监控：100 单/月约 8 个月撞 4MB 上限，提前 1-2 个月告警以便安排 IndexedDB 迁移。
      // 3MB 触发警告（继续写入，业务暂不受影响），4MB 硬切回退（跳过写入，转走 Path B）。
      if (str.length > 3 * 1024 * 1024) {
        console.warn('[cache] 业务缓存 >3MB（' + (str.length / 1024 / 1024).toFixed(2) + 'MB），建议改 IndexedDB。可在 DevTools → Application → Local Storage 查看 ds_biz_cache_v1 占用。');
      }
  ...
  function updateSync() {
    ...
    // 【v482】同步失败状态：后台 reload 抛错（401/网络/5xx）时填入 _syncError，指示器显式告警 + 红字 + cursor: pointer
    //   此前错误只到 console.warn，用户完全感知不到——订单停留在缓存数，指示器显示"已同步 X 分钟前"误以为成功。
    if (state._syncError) {
      if (elClock) elClock.textContent = '点击重试';
      if (elLabel) { elLabel.textContent = '同步失败'; elLabel.style.color = '#fca5a5'; }
      const titleEl = $('#syncStatus');
      if (titleEl) {
        titleEl.title = '后台拉取失败：' + (state._syncError.message || state._syncError) + '\n点击右上角刷新按钮重试';
        titleEl.style.cursor = 'pointer';
      }
      return;
    }
    const titleEl2 = $('#syncStatus');
    if (titleEl2) titleEl2.style.cursor = '';
    // 【v479b/v493】后台同步中：隐藏相对时间、改显示旋转 spinner，只保留「同步中」标签。
    //   避免 markSynced 之后、render 还没把新数据刷到 DOM 时出现的「刚刚 + 旧数据」错位；
    //   同时让「同步中 正在同步…」这行字更精简（文字 + 动画反馈）。
    if (state._isSyncing) {
      const elSpin = $('#syncSpin');
      if (elClock) elClock.style.display = 'none';
      if (elSpin) elSpin.style.display = 'inline-block';
      if (elLabel) { elLabel.textContent = '同步中'; elLabel.style.color = ''; }
      return;
    }
    // 非同步态：恢复文字时钟、隐藏 spinner
    const elSpin = $('#syncSpin');
    if (elSpin) elSpin.style.display = 'none';
    if (elClock) elClock.style.display = '';
    if (elClock) {
    ...
    if (elLabel) {
      const stale = !t || (now - (t || 0) > 24 * 60 * 60 * 1000); // 跨会话已保留真实同步时间：仅「从未同步」或「超24h」才告警，避免半天没开就一直显示未同步
  ...
  // 刷新按钮即时反馈：点下立刻显示「刷新中…」+ 旋转图标并禁用，云端数据真正回来才复位。
  // 这样不再有「点了没反应 / 假完成」的错觉（耗时是真实网络往返，无法抹掉，但至少看得见在转）。
  function setBtnRefreshing(btn, on) {
    if (!btn) return;
    if (on) {
      if (btn.dataset.busy) return;
      btn.dataset.busy = '1';
      btn.dataset.orig = btn.innerHTML;
      const dash = btn.id === 'btnDashRefresh';
      btn.innerHTML = dash
        ? '<span class="desktop-inline">刷新中…</span><span class="mobile-inline spinning">⟳</span>'
        : '<span class="desktop-only">刷新中…</span><span class="mobile-only spinning" aria-hidden="true">⟳</span>';
  ...
  // ⚠️ 关键修复：此前 refreshAll() 只从内存缓存（DB.list*）重渲染，并不真正向云端拉取，
  // 所以「点刷新显示已刷新、数据却还是老的」——内存里本来就是旧值，重渲染当然还是旧的。
  // 这里先 await DB.reload() 真正从服务端取最新（SW 现在对 API 走 network-first，不再被旧缓存糊弄），
  // 再 refreshAll() 重渲染，刷新才名副其实。"已刷新数据"提示也只在云端数据真正回来后才出现。
  async function doRefresh(silent) {
    if (state.currentUser) {
      // 【v479b】同步中：刷新期间显式锁定指示器，避免「刚刚 + 旧数据」短促出现
      // 【v482】清空上次的失败状态，updateSync 恢复"正在同步…"显示
      state._syncError = null;
      const wasSyncing = state._isSyncing;
      state._isSyncing = true; updateSync();
      try { await DB.reload(); }   // 真正从云端取最新数据
      catch (e) { console.warn('[refresh] 云端拉取失败，使用本地缓存渲染', e); }
      // 等 renderTabContent 把新数据落到 DOM 后再解锁
      await refreshAll();
      DB.markSynced();
      state._isSyncing = wasSyncing; // 恢复 bootApp 时设置的同步态（Path A 期间为 true，其他场景为 false）
      updateSync();
  ...
  async function switchTab(tab, opts) {
    ...
    if (track && isSwipeMode() && toIdx !== fromIdx) {
      ...
      // 【性能修复】先立即启动滑动动画（点击后页面立刻开始滑动，消除"点下去卡一下才切过去"的卡顿感），
      // 再把渲染放到动画进行中异步执行。目标页通常已被 preRenderNeighbors 预渲染，滑动到位时已有内容；
      // force:true 仍在后台保证数据最新，渲染失败由 ensureTabRendered 兜底补渲染。
      ...
      if (opts.render !== false) {
        renderTabContent(tab, { force: true }).catch(e => console.error('[switchTab] 渲染目标页失败：', tab, e));
      }
  ...
  function enhanceSelect(sel) {
    ...
    function open() {
      render();
      // 弹层改为 fixed 定位：脱离 .card{overflow:hidden} 的裁剪框，避免下拉在卡片底部展开时被父容器截断。
      // 移动端 #swipeTrack 常驻 transform，会把内部 fixed 子元素的坐标系带偏；
      // 因此把弹层提升到 body，让 fixed 定位真正基于 viewport。
      if (list.parentNode !== document.body) document.body.appendChild(list);
      const rect = btn.getBoundingClientRect();
      const vw = window.innerWidth, vh = window.innerHeight;
      list.style.minWidth = rect.width + 'px';
      const estH = list.children.length * 36 + 8;
      const below = vh - rect.bottom;
      const top = (below >= estH + 8) ? (rect.bottom + 4) : Math.max(8, rect.top - estH - 4);
      const left = Math.max(8, Math.min(rect.left, vw - rect.width - 8));
      list.style.top = top + 'px';
      list.style.left = left + 'px';
      list.classList.add('open');
      btn.classList.add('open');
      // 页面滚动/缩放会使 fixed 浮层错位，关闭以保位置正确；列表自身内部滚动（target 在 list 内）忽略
      scrollClose = (e) => { if (list.contains(e.target)) return; close(); };
    ...
    function close() {
      ...
      if (scrollClose) { window.removeEventListener('scroll', scrollClose, true); window.removeEventListener('resize', scrollClose); scrollClose = null; }
      // 关闭后把弹层移回原容器，保持 DOM 结构整洁；若原容器已被移除则留在 body 避免报错
      if (list.parentNode !== wrap && wrap.isConnected) wrap.appendChild(list);
    }
  ...
  async function checkAutoDispatch() {
    ...
    try {
      // 第一遍：用内存快筛候选，无候选直接返回（不联网，绝大多数情况到此为止）
      const localOrders = await DB.listOrders();
      const now = new Date();
      const mins = (st.auto_dispatch_minutes != null ? st.auto_dispatch_minutes : 5);
      const waitMs = mins * 60 * 1000;
      const hasCandidate = localOrders.some(o =>
        o.status === '接单' && o.assigned_designer_id && o.deadline && o.intake_at &&
        (now - new Date(o.intake_at) >= waitMs)
      );
      if (!hasCandidate) return;
      // 有候选：先刷新云端最新状态（后端 pg_cron 可能已把单派掉），避免用内存旧「接单」值
      // 重复派单 / 重复写 operation_logs。刷新后再判断，云端已是「派单」的单会被跳过。
      await DB.reload({live:true});   // 只拉订单/客户（派单只需订单），静态表用缓存
      const orders = await DB.listOrders(); // 刷新后的最新数据
      let dispatched = 0;
      for (const o of orders) {
        if (o.status !== '接单') continue; // 云端已是派单（后端已派）→ 跳过，不重复写日志
  ...
  // ========== 【v540】抢单平台(xinxifabu)新需求实时提醒 ==========
  // 订阅 dr_requirements 的 INSERT：新需求发布 → 未读+1 + toast 提醒（开关可关）
  function subscribeNewReqRealtime() {
    if (state._newReqChannel) return;                        // 防重复订阅
    try { state._newReqNotify = (localStorage.getItem('ds_newreq_notify') || '1') !== '0'; } catch (e) {}
    state._newReqChannel = DB.subscribeDrNewReq((row) => {
      if (!row) return;
      if (row.deleted_at) return;                            // 软删不计
      if (row.status && row.status !== 'open') return;       // 仅"可抢"状态提醒（locked/in_progress 等不打扰）
      if (!state._newReqNotify) return;                      // 开关关：静默，不计数不弹
      state._newReqUnread = (state._newReqUnread || 0) + 1;
      renderNewReqBadge();
      const tip = '📢 新需求：' + (row.task_type || '其他') + ' · ' + (row.title || row.publisher_name || '未命名需求');
      toast(tip);
    });
  }

  // 顶栏「新需求」铃铛角标：未读数>0 且开关开时显示红点数字
  function renderNewReqBadge() {
    const btn = $('#btnNewReq'); if (!btn) return;
    const badge = $('#newReqBadge'); if (!badge) return;
    const n = state._newReqUnread || 0;
    if (n > 0 && state._newReqNotify) {
      badge.style.display = '';
  ...
  function bindGlobal() {
    ...
    $('#btnRefresh').addEventListener('click', () => {
      ...
      setBtnRefreshing(btn, true);
      doRefresh()
        .then(() => {})
        .catch(e => toast('刷新失败：' + (e && e.message || e)))
        .finally(() => setBtnRefreshing(btn, false));
    });
    ...
    const btnNewReq = $('#btnNewReq');
    if (btnNewReq && !btnNewReq._v540Bound) {
      btnNewReq._v540Bound = true;
      btnNewReq.addEventListener('click', (e) => {
        e.preventDefault();
        openNewReqPanel();
      });
    }
    // 【v540】点击面板外任意处关闭提醒面板（btnNewReq 内点击不关闭）
    document.addEventListener('click', (e) => {
    ...
    if (nrNotify && !nrNotify._v540Bound) {
      nrNotify._v540Bound = true;
      nrNotify.addEventListener('change', (e) => {
        state._newReqNotify = !!e.target.checked;
        try { localStorage.setItem('ds_newreq_notify', state._newReqNotify ? '1' : '0'); } catch (err) {}
        if (!state._newReqNotify) { state._newReqUnread = 0; }
        renderNewReqBadge();
        toast(state._newReqNotify ? '已开启新需求提醒' : '已关闭新需求提醒');
      });
    }
    // 【v482】点击同步状态指示器：仅在「同步失败」状态下充当快速重试入口
    //   平时无视觉提示（cursor: default），不会和刷新按钮抢用户点击习惯
    const syncStatus = $('#syncStatus');
    if (syncStatus && !syncStatus._v482Bound) {
      syncStatus._v482Bound = true;
      syncStatus.addEventListener('click', () => {
        if (!state._syncError) return;   // 正常态不响应（避免误点）
        doRefresh();
      });
    }
    ...
    document.addEventListener('visibilitychange', async () => {
      ...
      _fgWasHidden = false;
      try {
        await DB.reload({live:true});   // 回前台：只拉订单/客户，静态表用缓存，缩短旧→新等待
        await loadData();
        updateSync();
    ...
    window.addEventListener('pageshow', async (e) => {
      ...
      try {
        await DB.reload({live:true});   // bfcache 恢复：只拉订单/客户，静态表用缓存
        await loadData();
        updateSync();
        if (state.tab) switchTab(state.tab, { render: true });
        checkAutoDispatch().catch(() => {});
  ...
  async function renderOrders() {
    ...
    const rows = pageItems.map(o => {
      ...
      const designerCell = (esc(dsMap[o.assigned_designer_id]) || softBadge('#64748b', '未派')) +
        (collabNames.length ? ' <span class="collab-tag" title="协作：' + collabNames.map(esc).join('/') + '">+' + collabNames.map(n => esc(n.slice(0, 1))).join('/') + '</span>' : '') +
    ...
    $('#ordersTable').innerHTML =
      '<thead><tr>' +
      sortable('order_no', '单号', 'center') + sortable('title', '项目') + sortable('customer_name', '客户') +
      sortable('task_type', '类型') + sortable('amount', '金额', 'num') + sortable('category', '分类', 'center') +
      '<th>设计师</th>' +       sortable('status', '状态', 'center') + sortable('deadline', '截稿', 'center') +
  ...
  async function openOrder(id) {
    ...
    // 【v479】主加载列已裁剪（不含 flow_history / complaint_log），详情弹窗打开时按需补拉一次全行。
    //   补拉到的字段会就地合并到 editingOrder（以及 cache），保证流程日志、投诉列表正常显示。
    renderOrderModal();
    DB.loadOrderDetail(id).then(detail => {
      if (!detail || state.editingOrder?.id !== id) return; // 用户已切到其他单/关弹窗
      state.editingOrder = Object.assign({}, state.editingOrder, {
        flow_history: detail.flow_history,
        complaint_log: detail.complaint_log
      });
      renderOrderModal();
    }).catch(e => console.warn('[openOrder] 补拉详情失败：', e));
  }
  ...
  function renderWorkbench() {
    ...
    // 设计返工率（新口径）：已定稿中经历过程改(revision_count>0)的占比（rework_category 已不再采集）
    const reworkedCount = finalizedAny.filter(o => (o.revision_count || 0) > 0).length;
    const reworkRate = finalizedAny.length ? reworkedCount / finalizedAny.length : 0;
    let cycSum = 0, cycN = 0;
  ...
  async function exportAnaCSV() {
    ...
    try {
      ...
      const orderHead = ['单号', '项目', '客户', '任务类型', '金额', '状态', '参与设计师',
        '改稿次数', '投诉', '周期(天)', '接单时间', '派单时间', '截稿时间', '定稿时间', '换人时间'];
      const orderRows = (rep.perOrder || []).map(o => [
        ...
        txt(o.participantNames.join(' / ')), txt('改稿 ' + (o.revision_count || 0) + ' 次' + (reworkDays(o) != null ? ' · ' + fmtCycle(reworkDays(o)) : '')), txt(complaintReasons(o) || (o.complaint_count || 0)),
      ...
      const orderCols = [18, 28, 18, 12, 12, 12, 22, 12, 14, 10, 16, 16, 16, 16, 16].map(w => ({ wch: w }));
  ...
  async function renderSettings() {
    ...
    // 【v540】新需求提醒开关：纯 UX 偏好，localStorage 持久化（非云端 settings）
    const nrNotifyEl = $('#sNewReqNotify');
    if (nrNotifyEl) {
      try { state._newReqNotify = (localStorage.getItem('ds_newreq_notify') || '1') !== '0'; } catch (e) {}
      nrNotifyEl.checked = !!state._newReqNotify;
    }
  ...
  async function renderAnalytics() {
    ...
    Charts.bar($('#chartRework'), {
      title: '设计返工率(%)（有修改 ÷ 已定稿）', horizontal: false,
      labels: names,
  ...
  function renderAnaProjects(rep) {
    ...
    const hasComplaint = list.some(o => o.complaint_count > 0);
    const tbl = $('#anaProjects');
    tbl.className = 'tbl' + (hasComplaint ? ' has-complaint' : '');
    tbl.innerHTML =
      '<thead><tr><th>单号</th><th>项目</th><th>客户</th><th>参与设计师</th><th>状态</th><th>改稿次数</th><th>投诉</th><th>周期(天)</th><th>金额</th></tr></thead><tbody>' +
      (list.length ? list.map(o =>
        '<tr><td>' + esc(o.order_no) + '</td><td>' + esc(o.title) + '</td><td>' + esc(o.customer_name || '') + '</td>' +
        '<td>' + esc(o.participantNames.join(' / ')) + '</td><td>' + pill(o.status) + '</td><td>' + (o.revision_count ? ('改稿 ' + o.revision_count + ' 次' + (reworkDays(o) != null ? ' · ' + fmtCycle(reworkDays(o)) : '')) : '<span style="color:var(--muted)">—</span>') + '</td>' +
        '<td>' + (o.complaint_count ? (() => {
          const cr = complaintReasons(o);
          const designComplaint = Array.isArray(o.complaint_log) && o.complaint_log.some(c => c.reason === '设计原因');
          return '<span class="badge ' + (designComplaint ? 'bad' : 'warn') + '" title="投诉 ' + o.complaint_count + ' 次">' + esc(cr || '—') + '</span>';
        })() : '0') + '</td>' +
        '<td class="num">' + (o.cycleDays != null ? fmtCycle(o.cycleDays) : '—') + '</td>' +
        '<td class="num">¥' + money(o.amount) + '</td></tr>'
      ).join('') : '<tr><td colspan="9" class="empty">范围内暂无订单</td></tr>') + '</tbody>';
  }
  ...
  function bootApp() {
    ...
    if (loggedIn && hasCache) {
      ...
      state.tab = 'dashboard';
      // 【v534】取消 v479b 的「_isSyncing=true」锁定：仪表盘已即时上屏、缓存同步时间可信，
      //   启动期间没必要把指示器钉在「同步中」等 6s（东京 loadLive 跨区耗时），
      //   否则用户感觉「数据出来了指示器却还卡转圈」。直接显示「已同步 <上次同步时间>」，
      //   6s 后 afterLogin(Path A) 末尾的 updateSync() 自动刷成「刚刚」。
      //   doRefresh 主动刷新仍设 _isSyncing=true（line 1898），保留「同步中」防「刚刚+旧数据」错位。
      hideSplash();                       // ← 同步关首屏，毫秒级，绝不等待网络
      updateSync();                       // 【v471】界面一出来即显示「已同步 <真实上次同步时间>」，无需等 bootAuth/reload 跑到 afterLogin 末尾才填
